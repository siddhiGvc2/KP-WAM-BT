                                ...
                                int32_t event_id, void* event_data)
{
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) 
    {
         if (gpio_get_level(JUMPER2) == 0)
         {
             ...
             // Dispaly Code C01 - remove jumper
             DisplayCode(CodeRemoveJumper);
             if(UartDebugInfo)
            ...
            xTaskCreate(smartconfig_example_task, "smartconfig_example_task", 4096, NULL, 6, NULL);
             // Dispaly Code C02 - Seraching for WiFi
             DisplayCode(CodeEspTouch);
