...
void load_settings_nvs(){
    ...
    ESP_LOGI(TAG, "*NVS Reading Started#");
      if(utils_nvs_get_int(NVS_SOUND_MODE,&SoundMode) == ESP_OK){
        utils_nvs_get_int(NVS_SOUND_MODE,&SoundMode);
      }
      else{
        SoundMode=0;
      }
      ...
      {
        ESP_LOGI(TAG,"Error in MAX COUNT reading is ");
      }
       if(utils_nvs_get_int(NVS_CURRENTCOUNT,&current_count) == ESP_OK){
        utils_nvs_get_int(NVS_CURRENTCOUNT,&current_count);
      ...
      if(utils_nvs_get_int(NVS_GAP_TIME,&GapBeforeNextLight) == ESP_OK)
      {
        utils_nvs_get_int(NVS_GAP_TIME,&GapBeforeNextLight); 
