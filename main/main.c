...
void Led_Task(void *pvParameters){
    ...
    while (1)
    {
        ...
        HalfTick ^= 1;
        // blink result when wait4nextgame is active
        if (HalfTick)
        {
            if (Wait4NextGame)
            {
  ...
  //          DisplayDigit4(0,0,Ticks);
            ...
            if (RemainingTime == 0)
//            if (((CurrentTime - LightOnTime) > TimeToStop) || RemainingTime == 0)
            {
                GameOverRoutine();
                // added on 050825
                current_count+=1;
                utils_nvs_set_int(NVS_CURRENTCOUNT,current_count);
...
void app_main(void)
{
    ...
    load_settings_nvs();
    // extract numeric portion of serial number - discard NA-1507- GVC-1125-XXXXX
...
//    StartPlaySoundRoutine();
    ...
    // }
    Init6961();
    DisplayDigit4(0,0,NumericSerialNumnber);
    DisplayDigit3(4,0,NumericFWVersion);
